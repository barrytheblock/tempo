(function () {
  ...
  const __storageCache = Object.create(null);

  function loadJSON(key, fallback) {
    try {
      const raw = Object.prototype.hasOwnProperty.call(__storageCache, key) ? __storageCache[key] : null;
  ...
  function saveJSON(key, value) {
    try {
      ...
      __storageCache[key] = raw;
      chrome.storage.local.set({ [key]: raw }, () => {
        if (chrome.runtime.lastError) {
          console.error('[Tempo] failed to save', key, chrome.runtime.lastError);
        }
      });
    } catch (e) {
  ...
  function bootstrapStorage(callback) {
    try {
      chrome.storage.local.get(null, (items) => {
        if (chrome.runtime.lastError) {
          console.error('[Tempo] failed to load storage', chrome.runtime.lastError);
        } else {
          Object.assign(__storageCache, items);
        }
        Object.assign(settings, loadJSON(NS + 'settings', {}));
        callback();
      });
    } catch (e) {
      console.error('[Tempo] failed to bootstrap storage', e);
      callback();
    }
  }
...
function buildAppOverrideCSS(theme) {
  ...
  return `
    ...
    /* Sidebar, Top Header, and Bottom Player Bar (Lighter background) */
    ...
    /* FIX: Search bar input box, background, and placeholder text */
    ...
    /* FIX: Icons and buttons in sidebar, header, and player bar (Hamburger, tab icons, player controls) */
    ...
    /* FIX: Now playing bar track title and secondary details */
    ...
    /* Fix all text elements inside the inner content area */
    ...
    /* Subtitles and secondary text descriptions */
    ...
    /* Keep album art and thumbnails cleanly visible */
  ...
  function exportShareImage() {
    try {
      ...
      ctx.fillText('via Tempo • by barrytheblock', 60, h - 40);
  ...
  function init() {
    ...
    // No GM_registerMenuCommand in a content script — instead, let the
    // extension's toolbar icon (background.js) open the panel on request.
    if (chrome.runtime && chrome.runtime.onMessage) {
      chrome.runtime.onMessage.addListener((message) => {
        if (message && message.type === 'ytt_toggle_panel') togglePanel();
      });
    }
  ...
  bootstrapStorage(startWhenReady);
})();